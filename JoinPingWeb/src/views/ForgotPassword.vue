...
<script>
// 密码重置页面组件
// 功能：用户输入手机号和新密码进行密码重置
// 说明：对接后端密码重置API，实现密码重置功能
...
export default {
  ...
  computed: {
    // 检查是否可以重置密码（所有字段都不为空且密码一致）
    canReset() {
      const result = (
        this.forgotPasswordForm.phone.length === 11 && 
        this.forgotPasswordForm.password.length >= 6 && 
        this.forgotPasswordForm.password === this.forgotPasswordForm.confirmPassword &&
        !this.isLoading
      );
      console.log('canReset计算:', {
        phone: this.forgotPasswordForm.phone,
        password: this.forgotPasswordForm.password,
        confirmPassword: this.forgotPasswordForm.confirmPassword,
  ...
  methods: {
      ...
      handleReset() {
        ...
        console.log('表单验证通过，开始调用密码重置API');
        ...
        userApi.forgetPassword(
          ...
          if (response.code === 200 && response.hasSuccessed) {
            ...
            // 显示错误信息
            const errorMsg = response.message || '密码重置失败，请稍后重试';
            console.error('密码重置失败:', errorMsg);
            this.$toast(errorMsg);
          }
        }).catch(error => {
      ...
      validateForm() {
        ...
        // 只校验手机号是否为11位
        if (this.forgotPasswordForm.phone.length !== 11) {
        ...
        // 后端验证：密码长度不超过20位
        if (this.forgotPasswordForm.password.length > 20) {
        ...
        // 验证确认密码
        if (!this.forgotPasswordForm.confirmPassword) {
          console.log('确认密码为空');
          this.$toast('请确认新密码');
          return false;
        }
